...
export default function MobilePromotion() {
  ...
  return (
    <>
      <Swiper spaceBetween={12} slidesPerView={"auto"} className="w-auto max-w-[100%] px-[12px]">
        {data.map((items) => {
            ...
            <SwiperSlide key={items.id} className="w-[240px] max-w-[240px]">
              ...
              >
                ...
                >
                  ...
                  <Flex width={"100%"} direction="column" gap={{ row: 8 }} className="text-white">
                    <p className="text-p1B w-full truncate">{items.name}</p>
                    <Flex width={"100%"} direction="column">
                      <Flex items="center" gap={{ column: 8 }} className="text-span1R">
                        <span>{items.event.startDate}</span>-<span>{items.event.endDate}</span>
                      </Flex>
                      <Flex items="center" gap={{ column: 8 }}>
                        <span className="text-span1R text-white">{items.category}</span>
                        <Divider type="vertical" height={8} color="bg-dark-300" />
                        <span className="text-span1R text-white">{items.event.ageLimit}</span>
                      </Flex>
                    </Flex>
                  </Flex>
                </Flex>
